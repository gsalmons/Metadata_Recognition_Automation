...
try:
    for train_index, test_index in skf.split(xRandomSample, yTruthList):
        ...
        plt.grid(True)
        plt.show()
        plt.savefig(f'/results/projectEmbedding/precision_recall_curve_allsub_{foldNumber}.png')
...
plt.grid(True)
plt.show()
plt.savefig('/results/projectEmbedding/precision_recall_curve.png')

with open("/results/kFoldTsvs/projectEmbeddingConfidencesallsub.tsv", "w") as writeFile:
...
plt.savefig('/results/projectEmbedding/confusion_matrix_allsub.png')
plt.show()